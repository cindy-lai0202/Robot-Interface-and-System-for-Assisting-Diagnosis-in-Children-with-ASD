...
public class MainActivity2 extends RobotActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        robotAPI.robot.setExpression(RobotFace.HIDEFACE,inputname+"，可以幫我找出" + str[0]+"跟"+str[1] + "嗎?");
        ...
        for (int i = 0; i < card.length; i++) {
            imgs[i].setImageResource(card[i]);
                if (card[i] == R.drawable.pineapple || card[i] == R.drawable.apple ) {
                    final int j = i;
                    imgs[i].setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            if(bingo == 2){
                                stopRecordingVideo();
                                robotAPI.robot.setExpression(RobotFace.ACTIVE,"答對瞜，你好棒!");
                                robotAPI.robot.speak("最後一題瞜!");
                                fileUri = Uri.fromFile(new File("/storage/emulated/0/Android/data/com.example.chooseimage_zenbo/files/"+name));
                                uploadVideo();
                                robotAPI.robot.speak("這次我需要三種水果");
                            }
                        }
                    });
                } else {
    ...
    private String getVideoFilePath(Context context) {
        ...
        name="chooseimage2_"+timeStamp+".mp4";
