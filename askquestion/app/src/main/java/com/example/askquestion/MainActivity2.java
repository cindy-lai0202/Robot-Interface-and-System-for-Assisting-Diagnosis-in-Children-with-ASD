...
public class MainActivity2 extends RobotActivity {
    ...
    int[] card2 = {R.drawable.spoon,R.drawable.knife,R.drawable.chopsticks};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //實體化
        robotAPI.robot.setExpression(RobotFace.HIDEFACE, "喝湯要用什麼喝呢？");
        for (int i = 0; i < 3; i++) {
        ...
        for (int i = 0; i < 5000; i++) {
            ...
            int c = card2[shuffle2];
            card2[shuffle2] = card2[0];
            card2[0] = c;
        }
        ...
        for (int i = 0; i < card2.length; i++) {
            imgs[i].setImageResource(card2[i]);
        }
            for (int i = 0; i < card2.length; i++) {
                if (card2[i] == R.drawable.spoon) {
                    imgs[i].setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            uploadVideo();
                            robotAPI.robot.setExpression(RobotFace.PROUD,"答對瞜，你好棒!");
                            robotAPI.robot.setExpression(RobotFace.PROUD,"我再來考考你");
                        }
                    });
                } else {
                    imgs[i].setOnClickListener(new View.OnClickListener() {
        ...
        try {
            ...
            cameraManager.openCamera(cameraBF, new CameraDevice.StateCallback() {
                ...
                public void onDisconnected(CameraDevice camera) {
                    if (cameraDevice != null) {
                        cameraDevice.close();
                        MainActivity2.this.cameraDevice = null;
                    }
                }

                @Override
                public void onError(CameraDevice camera, int error) {
                    Toast.makeText(MainActivity2.this, "Open Error", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }, null);
        } catch (CameraAccessException e) {
