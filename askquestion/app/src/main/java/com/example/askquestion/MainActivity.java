...
public class MainActivity extends RobotActivity {
    ...
    int[] card = {R.drawable.toothbrush,R.drawable.comb,R.drawable.smartphone};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        robotAPI.robot.setExpression(RobotFace.HIDEFACE, inputname + ",我來考考你喔");
        robotAPI.robot.setExpression(RobotFace.HIDEFACE, "刷牙要用什麼呢？");
        for (int i = 0; i < card.length; i++) {
            imgs[i].setImageResource(card[i]);
        }
            for (int i = 0; i < card.length; i++) {
                if (card[i] == R.drawable.toothbrush) {
                    imgs[i].setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            uploadVideo();
                            robotAPI.robot.setExpression(RobotFace.INTERESTED,"答對瞜，你好棒!");
                            robotAPI.robot.setExpression(RobotFace.INTERESTED,"再來第二題");
                        }
