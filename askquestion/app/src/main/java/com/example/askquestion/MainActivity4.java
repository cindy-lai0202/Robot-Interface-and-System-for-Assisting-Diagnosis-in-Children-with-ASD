...
public class MainActivity4 extends RobotActivity {
    ...
    ImageView[] imgs = {iv1,iv2,iv3};
    int[] card4 = {R.drawable.pencil,R.drawable.eraser,R.drawable.hairdryer};
    int[] view = {R.id.img1,R.id.img2,R.id.img3};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < 5000; i++) {
            int shuffle2 = (int) (Math.random() * 3);
            int c = card4[shuffle2];
            card4[shuffle2] = card4[0];
            card4[0] = c;
        }
            //question4
        robotAPI.robot.setExpression(RobotFace.HIDEFACE, "用什麼寫字？");
        for (int i = 0; i < card4.length; i++) {
            imgs[i].setImageResource(card4[i]);}
            for (int i = 0; i < card4.length; i++) {
                if (card4[i] == R.drawable.pencil) {
                    imgs[i].setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            stopRecordingVideo();
                            fileUri = Uri.fromFile(new File("/storage/emulated/0/Android/data/com.example.askquestion/files/" + name));
                            uploadVideo();
                            robotAPI.robot.setExpression(RobotFace.EXPECTING,"答對瞜，你好棒!");
                            robotAPI.robot.setExpression(RobotFace.EXPECTING,"全部都答對了!");
                        }
                    });
        ...
        try {
            ...
            cameraManager.openCamera(cameraBF, new CameraDevice.StateCallback() {
                ...
                public void onDisconnected(CameraDevice camera) {
                    if (cameraDevice != null) {
                        cameraDevice.close();
                        MainActivity4.this.cameraDevice = null;
                    }
                }

                @Override
                public void onError(CameraDevice camera, int error) {
                    Toast.makeText(MainActivity4.this, "Open Error", Toast.LENGTH_SHORT).show();
                    finish();
                }
            }, null);
        } catch (CameraAccessException e) {
